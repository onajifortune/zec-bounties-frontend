...
export function WalletTopupModal({ open, onOpenChange }: WalletTopupModalProps) {
  ...
  // Dummy Zcash wallet address
  const walletAddress = "t1TwkE7ygBjKrYkVwgXJxJ2FjSEgF7b9Y8y"
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Scan the QR code or use the wallet address below to deposit ZEC</DialogDescription>
        </DialogHeader>

        <div className="flex flex-col items-center gap-6 py-4">
          {/* QR Code Placeholder - In production, use qrcode.react library */}
          ...
          <div className="w-full">
            ...
            <div className="flex gap-2 items-center bg-muted rounded-lg p-3 border border-border">
              <code className="text-xs font-mono flex-1 break-all text-foreground">{walletAddress}</code>
        ...
        <div className="flex gap-2">
          ...
          <Button className="flex-1 bg-primary hover:bg-primary/90">View on Block Explorer</Button>
        </div>
      </DialogContent>
    </Dialog>
  )
